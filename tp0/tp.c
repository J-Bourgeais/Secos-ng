...
char * types[]={"","AVAILABLE", "RESERVED", "ACPI_RECLAIMABLE", "NVS"};
char *type(multiboot_uint32_t t){
  if(t>0 && t<5){
    return types[t];
  }
  return "Unknown type";
...
void tp() {
   ...
   while((uint32_t)entry < (info->mbi->mmap_addr + info->mbi->mmap_length)) {
     ...
     debug("[0x%llx - 0x%llx] %d %s\n", entry->addr, entry->addr+entry->len - 1, entry->type, type(entry->type));
      entry++;
   }
...
/*    } */
   ...
   debug("----------Q4-------------\n");
     ...
     /* juste au‑dessus de 128 MiB */
    uint32_t beyond_128mb = (uint32_t)0x08000000; /* 128 * 1024 * 1024 */

    volatile int *p = (volatile int *)beyond_128mb;

    debug("TEST OUT-OF-RANGE READ at 0x%p: value = 0x%x\n", (void*)p, *p);

    debug("Trying OUT-OF-RANGE WRITE (0xdeadbeef) at 0x%p ...\n", (void*)p);
    *p = 0xdeadbeef;
    debug("After write, read back: 0x%x\n", *p);

    //On arrive pas a écrire --> peut etre un jour lever un seg ault
