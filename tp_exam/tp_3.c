...
static void gdt_entry_init(int idx, uint32_t base, uint32_t limit, uint8_t type, uint8_t dpl, uint8_t is_code) {
...
void setup_segmentation(void) {
    ...
    gdt_entry_init(1, 0, 0xFFFFF, 11, 0, 1); // Code Ring 0
    gdt_entry_init(2, 0, 0xFFFFF, 3, 0, 0);  // Data/Stack Ring 0 (U1)
    gdt_entry_init(3, 0, 0xFFFFF, 3, 0, 0);  // Data/Stack Ring 0 (U2)

    // Segments Utilisateurs (Index 4-8)
    gdt_entry_init(4, 0, 0xFFFFF, 11, 3, 1); // Code U1
    gdt_entry_init(5, 0, 0xFFFFF, 3, 3, 0);  // Stack U1
    gdt_entry_init(6, 0, 0xFFFFF, 11, 3, 1); // Code U2
    gdt_entry_init(7, 0, 0xFFFFF, 3, 3, 0);  // Stack U2
    gdt_entry_init(8, 0, 0xFFFFF, 3, 3, 0);  // Shared Data
    
    gdt_entry_init(9, 0, 0xFFFFF, 3, 0, 0);  // Kernel Data global
