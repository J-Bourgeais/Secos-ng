...
void SECTION_USER user1() {
    ...
    while (1) {
        /* Active wait to slow down incrementation for visibility */
...
void setup_paging(void) {
    ...
    /* Map Physical address for counter initialization */
    pg_set_entry(&((pte32_t*)0x613000)[pt32_get_idx(0x802000)], PG_USR | PG_RW, page_get_nr(PA_SHARED_PHYS));

    /* 3. Task 1 Shared Virtual Mapping (VA_SHARED_U1) */
    ...
    /* Visibility for Task 2 structures in Task 1 (for cross-task stability) */
    ...
    /* 2. User 2 Stack (0x801000) and Counter access */
    ...
    /* 3. Task 2 Shared Virtual Mapping (VA_SHARED_U2) */
...
void setup_hardware(uint32_t tick_hz) {
    ...
    out(0xFE, PIC_IMR(PIC1)); // Enable IRQ0 (Timer)
...
void tp() {
    ...
    intr_init();
    debug("[TP] Hardware (PIT/PIC) ready\n");
    ...
    ctx->cs.raw  = (6 << 3) | 3; // Index 6, RPL 3
    ctx->ss.raw  = (7 << 3) | 3; // Index 7, RPL 3
    ...
    *(volatile uint32_t*)PA_SHARED_PHYS = 0;
    ...
    uint32_t u_cs  = (4 << 3) | 3; // Index 4, RPL 3
    uint32_t u_ss  = (5 << 3) | 3; // Index 5, RPL 3

    debug("[TP] Switching to User1 (EIP=0x%x, ESP=0x%x)...\n", (uint32_t)user1, u_esp);

    /* Fake an interrupt return to enter Ring 3 */
    asm volatile(
